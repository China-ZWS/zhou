...
+ (NSString *)jumpToBizPay:(id)datas;
{

    //调起微信支付
    PayReq* req             = PayReq.new;
    req.partnerId           = datas[@"partnerid"];
    req.prepayId            = datas[@"prepayid"];
    req.nonceStr            = datas[@"noncestr"];
    req.timeStamp           = [datas[@"timestamp"] intValue];
    req.package             = datas[@"package"];
    req.sign                = datas[@"sign"];
    [WXApi sendReq:req];
    //日志输出
    return [NSString stringWithFormat:@"appid=%@\npartid=%@\nprepayid=%@\nnoncestr=%@\ntimestamp=%ld\npackage=%@\nsign=%@",[datas objectForKey:@"appid"],req.partnerId,req.prepayId,req.nonceStr,(long)req.timeStamp,req.package,req.sign];
}
@end
